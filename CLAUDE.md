...
| `setup_cog.py` | The `/setup` hub launcher (`setup_hub`) + every feature wizard (foundations, birthdays, growth, storm, members, shiny tasks, etc.), reachable as hub buttons. Largest file in the repo. The wizard pieces they share live in `wizard_steps.py` and `wizard_time.py`, the storm wizard in `storm_setup.py` and its participation step in `storm_setup_participation.py`, the Buddy System wizard in `buddy_setup.py`, the train wizard in `train_setup.py` and `train_setup_rotation.py`, the survey wizard in `survey_setup.py`, the birthday wizard in `birthday_setup.py`, the Growth Breakdown wizard in `growth_breakdown_setup.py`, the Shiny Tasks wizard in `shiny_tasks_setup.py`; all are imported back by name. | ~2.2K |
