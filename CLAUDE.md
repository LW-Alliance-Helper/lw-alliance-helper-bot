...
| `storm_setup_structured.py` | The storm wizard's structured-flow step (Premium opt-in, power data source, sub mode, sign-up channel + schedule, tab names, power-refresh / stale-power DMs, roster DM templates, preset library), split out of `setup_cog.py` in the #589 step 11 refactor ([#589](https://github.com/LW-Alliance-Helper/lw-alliance-helper-bot/issues/589)). One `_ask_*` function per question, a `_Wizard` for the shared handles, one `_Abort` exit for cancel / timeout, one `_ChoiceView` behind every picker. It reaches back into `setup_cog` at call time for the shared wizard pieces, so `patch("setup_cog.X")` still works; `setup_cog` keeps the old name as an import alias. Characterized by `tests/integration/test_storm_setup_structured.py`. | ~1.2K |
| `storm_setup.py` | The storm setup wizard itself (`run_storm_setup`: re-entry summary, Sheet tab, teams and time slots, log and post channels, per-team templates with Keep current, the participation and structured steps, reminder DM, save, summary, first sign-up offer), moved out of `setup_cog.py` in round 2 ([#611](https://github.com/LW-Alliance-Helper/lw-alliance-helper-bot/issues/611)). Same shape as `storm_setup_structured.py`: `_Wizard` / `_Saved` / `_Answers`, one function per step, one `_Abort` exit; the four views are module classes under their old names. Reaches back into `setup_cog` at call time for what the tests patch there; `setup_cog` imports the entry point back. Characterized by `tests/integration/test_storm_setup.py`, views clicked in `tests/unit/test_storm_setup_views.py`. | ~1K |
