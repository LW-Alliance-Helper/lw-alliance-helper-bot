...
def _channel_remap(field: str, purpose_key: str, value: int,
                   channel_lookup: Callable[[int], str],
                   purpose_format: dict | None = None) -> dict | None:
...
def _role_remap(field: str, purpose_key: str, value: int,
...
def collect_core(guild_id: int, *,
    ...
    channel_fields = [
        ("leadership_channel_id",     cfg.leadership_channel_id,
         "core.leadership_channel_id"),
        ("announcement_channel_id",   cfg.announcement_channel_id,
         "core.announcement_channel_id"),
        ("survey_channel_id",         cfg.survey_channel_id,
         "core.survey_channel_id"),
        ("survey_notify_channel_id",  cfg.survey_notify_channel_id,
         "core.survey_notify_channel_id"),
        ("ds_log_channel_id",         cfg.ds_log_channel_id,
         "core.ds_log_channel_id"),
        ("cs_log_channel_id",         cfg.cs_log_channel_id,
         "core.cs_log_channel_id"),
        ("event_draft_channel_id",    cfg.event_draft_channel_id,
         "core.event_draft_channel_id"),
        ("event_announce_channel_id", cfg.event_announce_channel_id,
    ...
    role_entry = _role_remap("member_role_id", "core.member_role_id",
                             cfg.member_role_id, role_lookup)
    if role_entry:
...
def collect_events(guild_id: int, *,
                   channel_lookup: Callable[[int], str],
                   role_lookup: Callable[[int], str]) -> list[dict] | None:
    ...
    for ev in rows:
        ...
        remap_channels = [
            entry for entry in (
                _channel_remap("draft_channel_id",
                               "events.draft_channel_id",
                               int(ev["draft_channel_id"] or 0),
                               channel_lookup, purpose_format=fmt),
                _channel_remap("announcement_channel_id",
...
def _collect_storm(guild_id: int, event_kind: str, *,
                   channel_lookup: Callable[[int], str],
                   role_lookup: Callable[[int], str]) -> list[dict] | None:
    ...
    for event_type in wanted:
        ...
        travels = {
            ...
            "participation_roster_name_col":
                int(d.get("participation_roster_name_col") or 0),
            "participation_roster_alias_col":
                int(d.get("participation_roster_alias_col") or -1),
            "participation_roster_start_row":
                int(d.get("participation_roster_start_row") or 2),
        }
        fmt = {"event_label": label}
        remap_channels = [
            entry for entry in (
                _channel_remap("log_channel_id",
                               "storm.log_channel_id",
                               int(d.get("log_channel_id") or 0),
                               channel_lookup, purpose_format=fmt),
                _channel_remap("post_channel_id",
...
def collect_train(guild_id: int, *,
                  channel_lookup: Callable[[int], str],
                  role_lookup: Callable[[int], str]) -> dict | None:
    ...
    remap_channels = [
        entry for entry in (
            _channel_remap("reminder_channel_id",
...
def collect_birthday(guild_id: int, *,
                     channel_lookup: Callable[[int], str],
                     role_lookup: Callable[[int], str]) -> dict | None:
    from config import _get_conn
    with _get_conn() as conn:
...
def collect_growth(guild_id: int, *,
                   channel_lookup: Callable[[int], str],
                   role_lookup: Callable[[int], str]) -> dict | None:
    from config import _get_conn
    with _get_conn() as conn:
    ...
    remap_channels = [
        entry for entry in (
            _channel_remap("breakdown_post_channel_id",
...
def collect_surveys(guild_id: int, *,
                    channel_lookup: Callable[[int], str],
                    role_lookup: Callable[[int], str]) -> list[dict] | None:
    ...
    for row in extra_rows:
        ...
        remap_channels = [
            entry for entry in (
                _channel_remap("survey_channel_id",
                               "surveys.survey_channel_id",
                               int(d.get("survey_channel_id") or 0),
                               channel_lookup, purpose_format=fmt),
                _channel_remap("notify_channel_id",
...
def collect_member_roster(guild_id: int, *,
    ...
    remap_roles: list[dict] = []
    role_entry = _role_remap("role_filter_id", "member_roster.role_filter_id",
...
def collect_available_categories(guild_id: int, *,
                                 channel_lookup: Callable[[int], str],
                                 role_lookup: Callable[[int], str],
                                 ) -> list[str]:
    ...
    return [
        ...
        if COLLECTORS[key](guild_id,
                           channel_lookup=channel_lookup,
                           role_lookup=role_lookup) is not None
    ]


def build_export(guild_id: int, *,
                 categories: list[str],
                 source_guild_name: str,
                 exporter_user_id: int,
                 channel_lookup: Callable[[int], str],
                 role_lookup: Callable[[int], str]) -> dict:
    ...
    for key in categories:
        if key not in COLLECTORS:
            continue
        collected = COLLECTORS[key](guild_id,
                                    channel_lookup=channel_lookup,
...
def parse_and_validate(file_bytes: bytes) -> dict:
    ...
    if not isinstance(source_guild, dict):
        raise ImportValidationError(
            "`source_guild` isn't an object. File is corrupted."
        )
    ...
    if not isinstance(data, dict):
        raise ImportValidationError(
            "`data` must be an object keyed by category name."
        )
        ...
        if cat in ("core", "train", "birthday", "growth", "shiny_tasks", "member_roster"):
            ...
            for key in ("travels", "remap_channels", "remap_roles"):
                if key in section and not isinstance(
                    ...
                    raise ImportValidationError(
                        f"`data.{cat}.{key}` has the wrong type."
                    )
        else:
            ...
            for i, row in enumerate(section):
                if not isinstance(row, dict):
                    raise ImportValidationError(
                        f"`data.{cat}[{i}]` must be an object."
                    )
...
def _resolve_channel(source_id: int, decisions: dict[int, tuple],
...
def _resolve_role(source_id: int, decisions: dict[int, tuple],
                  current_value: int) -> int:
    return _resolve_channel(source_id, decisions, current_value)


def apply_import(guild_id: int, parsed: dict,
                 remap: RemapDecisions) -> dict:
    ...
    if parsed.get("unknown_keys"):
        for k in parsed["unknown_keys"]:
            summary["warnings"].append(
                f"Ignored unknown key `{k}` (newer-than-this-bot data, "
                f"or hand-edited file)."
            )
    ...
    if "core" in cats:
        def _do_core():
            ...
            for field in ("leadership_role_name", "member_role_name",
                          "timezone", "event_draft_time",
                          "tab_train_schedule", "tab_ds_assignments",
                          "tab_sitouts", "tab_survey_history",
                          "tab_member_default"):
                if field in travels:
    ...
    if "events" in cats:
        def _do_events():
            from config import (
                get_guild_events, delete_guild_event, save_guild_event,
            )
            existing_by_key = {
            ...
            for row in data["events"]:
                ...
                current_draft = int(existing_by_key.get(t.get("short_key"), {}).get("draft_channel_id") or 0) \
                    if t.get("short_key") in existing_by_key else 0
                current_announce = int(existing_by_key.get(t.get("short_key"), {}).get("announcement_channel_id") or 0) \
                    if t.get("short_key") in existing_by_key else 0
                draft_id = (_resolve_channel(int(draft_entry["source_id"]),
                                             remap.channel_decisions,
                                             current_draft)
                            if draft_entry else current_draft)
                announce_id = (_resolve_channel(int(announce_entry["source_id"]),
                                                remap.channel_decisions,
                                                current_announce)
                               if announce_entry else current_announce)
    ...
    for storm_key, event_kind in (("ds", "DS"), ("cs", "CS")):
        ...
        def _do_storm(k=storm_key, kind=event_kind):
            ...
            for row in data[k]:
                ...
                log_id  = (_resolve_channel(int(log_entry["source_id"]),
                                             remap.channel_decisions, current_log)
                           if log_entry else current_log)
                post_id = (_resolve_channel(int(post_entry["source_id"]),
                                             remap.channel_decisions, current_post)
                           if post_entry else current_post)
    ...
    if "train" in cats:
        def _do_train():
            ...
            channel_remap = {entry["field"]: entry
                             for entry in data["train"].get("remap_channels") or []}
            with _get_conn() as conn:
                cur_row = conn.execute(
                    "SELECT reminder_channel_id FROM guild_train_config "
                    "WHERE guild_id = ?", (guild_id,),
                ).fetchone()
            cur_dict = dict(cur_row) if cur_row else {}
            current_reminder = int(cur_dict.get("reminder_channel_id") or 0)
            reminder_entry = channel_remap.get("reminder_channel_id")
            reminder_id = (_resolve_channel(int(reminder_entry["source_id"]),
                                            remap.channel_decisions,
                                            current_reminder)
                           if reminder_entry else current_reminder)
            try:
                templates = _json.loads(t.get("templates_json") or "[]")
            except (json.JSONDecodeError, TypeError):
                templates = []
    ...
    if "birthday" in cats:
        def _do_birthday():
            ...
            channel_remap = {entry["field"]: entry
                             for entry in data["birthday"].get("remap_channels") or []}
            with _get_conn() as conn:
                cur_row = conn.execute(
                    "SELECT reminder_channel_id FROM guild_birthday_config "
                    "WHERE guild_id = ?", (guild_id,),
                ).fetchone()
            cur_dict = dict(cur_row) if cur_row else {}
            current_reminder = int(cur_dict.get("reminder_channel_id") or 0)
    ...
    if "growth" in cats:
        def _do_growth():
            ...
            channel_remap = {entry["field"]: entry
                             for entry in data["growth"].get("remap_channels") or []}
            with _get_conn() as conn:
                cur_row = conn.execute(
                    "SELECT breakdown_post_channel_id FROM guild_growth_config "
                    "WHERE guild_id = ?", (guild_id,),
                ).fetchone()
            cur_dict = dict(cur_row) if cur_row else {}
    ...
    if "surveys" in cats:
        def _do_surveys():
            ...
            for row in data["surveys"]:
                ...
                channel_remap = {entry["field"]: entry
                                 for entry in row.get("remap_channels") or []}
                try:
                ...
                rem_id = (_resolve_channel(int(rem_entry["source_id"]),
                                            remap.channel_decisions,
                                            int(cur.get("reminder_channel_id") or 0))
                          if rem_entry else int(cur.get("reminder_channel_id") or 0))
                ...
                if survey_id != "default":
                    ...
                    s_id = (_resolve_channel(int(s_entry["source_id"]),
                                             remap.channel_decisions,
                                             int(cur.get("survey_channel_id") or 0))
                            if s_entry else int(cur.get("survey_channel_id") or 0))
                    n_entry = channel_remap.get("notify_channel_id")
                    n_id = (_resolve_channel(int(n_entry["source_id"]),
                                             remap.channel_decisions,
                                             int(cur.get("notify_channel_id") or 0))
                            if n_entry else int(cur.get("notify_channel_id") or 0))
                    with _get_conn() as conn:
                        conn.execute(
    ...
    if "member_roster" in cats:
        def _do_roster():
            ...
            with _get_conn() as conn:
                cur_row = conn.execute(
                    "SELECT role_filter_id FROM guild_member_roster_config "
                    "WHERE guild_id = ?", (guild_id,),
                ).fetchone()
            cur_dict = dict(cur_row) if cur_row else {}
            current_role = int(cur_dict.get("role_filter_id") or 0)
            role_entry = next((e for e in role_remap_entries if e["field"] == "role_filter_id"), None)
            role_filter_id = (_resolve_role(int(role_entry["source_id"]),
                                            remap.role_decisions,
                                            current_role)
                              if role_entry else current_role)
