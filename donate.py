"""
donate.py — /donate, /upgrade, and the premium-assignment commands.
...
Premium licensing model (see `premium.py` and issue #41):
  ...
  guild at a time" via an assignment layer; these commands are the
...
Commands defined here:
  ...
  - /premium_assign   — pin the caller's license to the current guild
  - /premium_status   — show subscription state and assigned guild
  - /premium_unassign — release the current assignment without canceling
"""
...
class DonateCog(commands.Cog):
    def __init__(self, bot: commands.Bot):
        ...
        # checkout. Set by /upgrade when the Discord premium button is
        # offered, consumed by on_entitlement_create. In-memory is fine —
        # a Railway restart that straddles a checkout just means the user
        # has to run /premium_assign manually after subscribing, which the
        # post-subscribe DM tells them to do anyway.
    ...
    async def upgrade(self, interaction: discord.Interaction):
        ...
        if caller_has_sub:
            ...
            if assigned_guild is None:
                ...
                if holder is not None and holder != user_id:
                    ...
                    embed = discord.Embed(
                        ...
                        description=(
                            ...
                            "(run `/premium_assign` there)\n"
                            "• Run `/premium_status` to see your options\n"
                        ),
                ...
                embed = discord.Embed(
                    ...
                    description=(
                        ...
                        "`/premium_assign` from that server, or `/premium_status` "
                        "to manage it."
                    ),
                    color=discord.Color.gold(),
                )
            ...
            if assigned_guild == guild_id:
                ...
                embed = discord.Embed(
                    ...
                    description=(
                        ...
                        "the cache to refresh, or run `/premium_status` to see "
                        "the current state."
                    ),
                    color=discord.Color.gold(),
                )
            ...
            embed = discord.Embed(
                ...
                description=(
                    ...
                    "A subscription can only be active in one server at a time. "
                    "Run `/premium_assign` here to switch, or `/premium_status` "
                    "to manage it."
                ),
        ...
        if view is not None:
            ...
            if guild_id is not None:
                self._pending_upgrade_guilds[user_id] = guild_id
    ...
    async def premium_assign(self, interaction: discord.Interaction):
        ...
        if not has_sub:
            embed = discord.Embed(
                ...
                description=(
                    "Run `/upgrade` to subscribe, then `/premium_assign` to pin "
        ...
        if current == guild_id:
            embed = discord.Embed(
                ...
                description=(
                    "Your subscription is already pinned to this server. "
                    "Run `/premium_status` to see details or `/premium_unassign` "
                    "to release it."
                ),
                color=discord.Color.gold(),
            )
        ...
        if holder is not None and holder != user_id:
            ...
            embed = discord.Embed(
                ...
                description=(
                    ...
                    "• Run `/premium_status` to see where your subscription can apply"
                ),
                color=discord.Color.orange(),
            )
            await interaction.response.send_message(embed=embed, ephemeral=True)
        ...
        confirm_embed = discord.Embed(
            ...
            description=(
                ...
                "to Free. You can switch again any time with `/premium_assign`."
            ),
            color=discord.Color.gold(),
        )
        await interaction.followup.send(embed=confirm_embed, ephemeral=True)
    ...
    @app_commands.command(
        name="premium_status",
        description="Show your Premium subscription and where it's currently assigned",
    )
    async def premium_status(self, interaction: discord.Interaction):
        ...
        if not has_sub and assigned_guild is None:
            embed = discord.Embed(
                ...
                description=(
                    "You don't have an active LW Alliance Helper Premium subscription. "
                    "Run `/upgrade` to start one."
                ),
                color=discord.Color.purple(),
            )
            await interaction.response.send_message(embed=embed, ephemeral=True)
        ...
        if not has_sub and assigned_guild is not None:
            ...
            embed = discord.Embed(
                ...
                description=(
                    ...
                    "automatically. To clear the assignment, run `/premium_unassign`."
                ),
                color=discord.Color.orange(),
            )
            await interaction.response.send_message(embed=embed, ephemeral=True)
        ...
        if assigned_guild is None:
            embed = discord.Embed(
                ...
                description=(
                    ...
                    "Run `/premium_assign` from the server you want Premium in."
                ),
                color=discord.Color.gold(),
            )
            await interaction.response.send_message(embed=embed, ephemeral=True)
        ...
        embed = discord.Embed(
            ...
            description=(
                ...
                "To move it, run `/premium_assign` from a different server. "
                "To release the pin without canceling, run `/premium_unassign`."
            ),
            color=discord.Color.gold(),
        )
        await interaction.response.send_message(embed=embed, ephemeral=True)
    ...
    async def premium_unassign(self, interaction: discord.Interaction):
        ...
        if current is None:
            embed = discord.Embed(
                ...
                description=(
                    ...
                    "`/premium_status` to see your subscription state."
                ),
                color=discord.Color.purple(),
            )
            await interaction.response.send_message(embed=embed, ephemeral=True)
        ...
        confirm_embed = discord.Embed(
            ...
            description=(
                ...
                "still active — run `/premium_assign` from any server you want "
                "to use it in next."
            ),
            color=discord.Color.orange(),
        )
    ...
    async def _dm_user(self, user_id: int, embed: discord.Embed) -> None:
        try:
            ...
            print(f"[PREMIUM] Cannot DM user {user_id} (DMs closed); "
                  f"they will need to run /premium_status manually")
    ...
    async def _dm_auto_assigned(self, user_id: int, guild_id: int) -> None:
        ...
        embed = discord.Embed(
            ...
            description=(
                ...
                f"different alliance later, run `/premium_assign` from that "
                f"server, or `/premium_status` to manage it."
            ),
            color=discord.Color.gold(),
        )
    ...
    async def _dm_blocked_assignment(self, user_id: int, guild_id: int, holder_id: int) -> None:
        ...
        embed = discord.Embed(
            ...
            description=(
                ...
                f"yours there. Run `/premium_assign` from a different "
                f"alliance Discord to pin your subscription, or "
                f"`/premium_status` to manage it later."
            ),
            color=discord.Color.orange(),
        )
        await self._dm_user(user_id, embed)
