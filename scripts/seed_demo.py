...
def seed_growth(args) -> None:
    ...
    print("  ✓ growth config written (5 metrics)")
...
def _build_storm_assignment_rows(event_type: str, zones_a, subs_a, zones_b, subs_b) -> list:
...
def seed_sheet(args) -> None:
    ...
    rows = [["Name", "1st Squad Power", "2nd Squad Power", "3rd Squad Power", "THP", "Total Kills"]]
    ...
    for offset in range(-7, 7):
        ...
        blurb = (
            f"{member} drives the train today — pile in!"
            if offset >= 0
            else f"{member} drove the train."
        )
        rows.append([d.isoformat(), member, blurb])
    ws.update(values=rows, range_name="A1")
    print("  ✓ Train Schedule tab — 14 days (7 past, 7 upcoming)")
    ...
    ws.update(values=rows, range_name="A1")
    print("  ✓ Survey History tab — 6 weeks of submissions")
    ...
    growth_metrics = ["1st Squad Power", "2nd Squad Power", "3rd Squad Power", "THP", "Total Kills"]
    ...
    ws.update(values=rows, range_name="A1")
    print(
        f"  ✓ Growth Tracking tab — {len(growth_periods)} periods × {len(growth_metrics)} metrics"
    )
    ...
    print("  ✓ DS Assignments tab — Team A + Team B rosters populated")
    ...
    print("  ✓ CS Assignments tab — Team A + Team B rosters populated")
...
def seed_demo_guild_from_env() -> None:
    ...
    if missing:
        ...
        print("[SEED] Aborting — set them in Railway and redeploy.")
        return
    ...
    if args.skip_sheet:
        print("[SEED] Sheet writes skipped (SEED_DEMO_SKIP_SHEET=1).")
    else:
        print("[SEED] Google Sheet:")
        try:
            seed_sheet(args)
        except Exception as e:
            print(f"[SEED]   ✗ Sheet write failed: {type(e).__name__}: {e}")
            print("[SEED]   (DB seeding succeeded; re-run with proper creds or set")
            print("[SEED]    SEED_DEMO_SKIP_SHEET=1 and populate the Sheet manually.)")
            return

    print("[SEED] Done. Set SEED_DEMO_ON_BOOT=0 (or remove the env vars) for the next deploy.")
...
def main() -> int:
    ...
    p.add_argument(
        "--guild-id", type=int, required=True, help="Discord guild ID of the demo server"
    )
    p.add_argument(
        "--sheet-id", type=str, required=True, help="Google Sheet ID for the demo alliance"
    )
    p.add_argument(
        "--leadership-channel",
        type=int,
    ...
    p.add_argument(
        "--leadership-role", type=int, required=True, help="Role ID for the leadership role"
    )
    p.add_argument(
    ...
    p.add_argument(
        "--member-role", type=int, required=True, help="Role ID for the alliance-member role"
    )
    p.add_argument(
        "--member-role-name",
        type=str,
    ...
    if args.skip_sheet:
        ...
        try:
            ...
            print("  (DB seeding succeeded — re-run with proper creds to fill the Sheet,")
            print("   or pass --skip-sheet and populate it manually.)")
            return 1

    print()
    print("Done. Next steps:")
    print("  1. In Discord, run /setup in your demo server's leadership channel")
    print("     and click 🗂️ View configuration to confirm everything looks wired up.")
    print("  2. Take screenshots of /help, /train overview, /birthdays, /desertstorm, etc.")
    print("  3. Re-run this script any time the demo data drifts (it's idempotent).")
    return 0
