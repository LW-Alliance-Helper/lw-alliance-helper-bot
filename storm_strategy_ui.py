...
create / edit / delete and exposes a per-preset editor.
...
Imports storm_strategy as `ss` for the data layer (ZoneRow, PresetBuffer,
load/save/list/delete_preset, canonical_zones_for, etc).
"""
...
class _ZoneEditModal(discord.ui.Modal):
    ...
    def __init__(self, view: "_PresetEditorView", zone_name: str):
        ...
        existing = view.buf.find_zone(zone_name) or ss.ZoneRow(zone=zone_name)
        ...
        if self._teams == "both":
            self.power_a_input = discord.ui.TextInput(
                ...
                placeholder="e.g. 80M",
                default=ss.format_power(existing.min_power_a) if existing.min_power_a else "",
                required=False,
                max_length=12,
            )
            ...
            self.power_b_input = discord.ui.TextInput(
                ...
                placeholder="e.g. 60M",
                default=ss.format_power(existing.min_power_b) if existing.min_power_b else "",
                required=False,
                max_length=12,
            )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        existing = self._view.buf.find_zone(self._zone_name) or ss.ZoneRow(zone=self._zone_name)
        ...
        if self.power_a_input is not None:
            min_a, bad_a = ss._parse_power_cell(self.power_a_input.value or "")
        else:
            min_a, bad_a = (existing.min_power_a or 0), False
        if self.power_b_input is not None:
            min_b, bad_b = ss._parse_power_cell(self.power_b_input.value or "")
        else:
        ...
        siblings = ss._sibling_zone_names(self._view.buf.zones, self._zone_name)
...
# submits, the accumulated values land on the ss.PresetBuffer in one
...
class _ZonePhaseCapacityAndFloorsModal(discord.ui.Modal):
    ...
    def __init__(self, view: "_PresetEditorView", zone_name: str):
        ...
        if self._teams in ("both", "A"):
            self.power_a_input = discord.ui.TextInput(
                label="Min Power Team A",
                placeholder="e.g. 80M",
                default=ss.format_power(pending["min_power_a"]) if pending["min_power_a"] else "",
                required=False,
                max_length=12,
            )
            self.add_item(self.power_a_input)
        if self._teams in ("both", "B"):
            self.power_b_input = discord.ui.TextInput(
                label="Min Power Team B",
                placeholder="e.g. 60M",
                default=ss.format_power(pending["min_power_b"]) if pending["min_power_b"] else "",
                required=False,
                max_length=12,
            )
            self.add_item(self.power_b_input)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if self.power_a_input is not None:
            val, bad = ss._parse_power_cell(self.power_a_input.value or "")
            if bad:
                await interaction.response.send_message(
        ...
        if self.power_b_input is not None:
            val, bad = ss._parse_power_cell(self.power_b_input.value or "")
            if bad:
                await interaction.response.send_message(
...
class _ZonePhasePriorityModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # Finalise: write the accumulated values to the ss.PresetBuffer.
        existing = self._view.buf.find_zone(self._zone_name) or ss.ZoneRow(zone=self._zone_name)
        self._view.buf.upsert_zone(
            ss.ZoneRow(
                zone=self._zone_name,
...
# `build_ds_strategy_group` / `build_cs_strategy_group` factories
# registered as subgroups under `/desertstorm` and `/canyonstorm`.
# Removed in the #187 follow-up — the hub buttons (`storm_event_hub`)
# call `open_strategy_list` and the view's inline Create / Edit /
# Delete buttons directly, so the factory layer is no longer reached.
