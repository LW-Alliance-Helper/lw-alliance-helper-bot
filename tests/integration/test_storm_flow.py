...
class TestDsMailGeneration:
    ...
    def test_full_ds_mail_a_contains_all_zones(self, seeded_db):
        ...
        save_storm_config(
            TEST_GUILD_ID, "DS", "DS Assignments", DEFAULT_DS_TEMPLATE, "America/New_York", 0
        )

        result = build_ds_mail(
            "A", SAMPLE_DS_ZONES, SAMPLE_DS_SUBS, "18:00 Server Time", guild_id=TEST_GUILD_ID
        )
    ...
    def test_ds_mail_contains_time(self, seeded_db):
        ...
        result = build_ds_mail(
            "A", SAMPLE_DS_ZONES, [], "18:00 Server Time", guild_id=TEST_GUILD_ID
        )
    ...
    def test_ds_mail_team_b_uses_b_template(self, seeded_db):
        ...
        save_storm_config(TEST_GUILD_ID, "DS_A", "DS Tab", "Team A: {zones}", "America/New_York", 0)
        save_storm_config(TEST_GUILD_ID, "DS_B", "DS Tab", "Team B: {zones}", "America/New_York", 0)
        save_storm_config(TEST_GUILD_ID, "DS", "DS Tab", "DS: {zones}", "America/New_York", 0)
...
class TestCsMailGeneration:
    ...
    def test_full_cs_mail_contains_all_zones(self, seeded_db):
        ...
        save_storm_config(
            TEST_GUILD_ID, "CS", "CS Assignments", DEFAULT_CS_TEMPLATE, "America/New_York", 0
        )

        result = build_cs_mail("A", SAMPLE_CS_ZONES, "12:00 Server Time", guild_id=TEST_GUILD_ID)
        assert "Alice" in result
        assert "Bob" in result
        assert "Carol" in result
    ...
    def test_cs_mail_contains_time(self, seeded_db):
        ...
        result = build_cs_mail("A", {"Z1": ["Alice"]}, "12:00 Server Time", guild_id=TEST_GUILD_ID)
    ...
    def test_cs_mail_uses_subs_not_subs_list(self, seeded_db):
        ...
        result = build_cs_mail("A", {"Z1": ["Alice"]}, "12:00", guild_id=TEST_GUILD_ID)
