...
class TestSurveySheetWrites:
    ...
    def test_update_squad_powers_creates_header_row(self, seeded_db, squad_powers_tab):
        ...
        save_survey_config(TEST_GUILD_ID, tab_name, "_unused_history", questions, "Test survey")
    ...
    def test_update_squad_powers_updates_existing_row(self, seeded_db, squad_powers_tab):
        ...
        with patch("survey._get_spreadsheet", return_value=sh):
            # First submission
            update_squad_powers("222222", "Alice", {"power": "40.00"}, guild_id=TEST_GUILD_ID)
            time.sleep(1)
            # Second submission — same user, different value
            update_squad_powers("222222", "Alice", {"power": "43.27"}, guild_id=TEST_GUILD_ID)
    ...
    def test_append_survey_history_creates_timestamped_rows(
        ...
        save_survey_config(TEST_GUILD_ID, sp_tab_name, hist_tab_name, questions, "")
        ...
        with patch("survey._get_spreadsheet", return_value=sh):
            append_survey_history("333333", "Bob", {"power": "38.50"}, guild_id=TEST_GUILD_ID)
            time.sleep(0.5)
            append_survey_history("444444", "Carol", {"power": "41.00"}, guild_id=TEST_GUILD_ID)
    ...
    def test_multiple_users_each_get_own_row(self, seeded_db, squad_powers_tab):
        ws, tab_name = squad_powers_tab
        from survey import update_squad_powers
        from config import save_survey_config
        ...
        users = [("111", "Alice", "43.27"), ("222", "Bob", "38.50"), ("333", "Carol", "41.00")]

        with patch("survey._get_spreadsheet", return_value=sh):
            for uid, name, power in users:
                update_squad_powers(uid, name, {"power": power}, guild_id=TEST_GUILD_ID)
                time.sleep(0.3)
