...
def stub_entry():
    ...
    release_announcements.RELEASE_ANNOUNCEMENTS["9.9.9"] = (
        release_announcements.ReleaseAnnouncement(
...
class TestMaybePostReleaseAnnouncement:
    async def test_posts_on_happy_path_and_updates_last_seen(self, temp_db, stub_entry):
        ...
        config.upsert_guild_install_metadata(
            ...
            current_version="9.8.0",  # one minor behind
        )
        config.save_config(
            config.GuildConfig(
                guild_id=12345,
                leadership_channel_id=999,
    ...
    async def test_skips_when_no_leadership_channel_and_does_not_bump(self, temp_db, stub_entry):
        ...
        config.save_config(
            config.GuildConfig(
                guild_id=12345,
                leadership_channel_id=0,  # never set up
                release_announcements_enabled=1,
            )
        )
    ...
    async def test_skips_when_channel_missing_from_guild_and_does_not_bump(
        self, temp_db, stub_entry
    ):
        import config
    ...
    async def test_skips_when_no_dict_entry_and_still_bumps_version(self, temp_db):
        ...
        await release_announcements.maybe_post_release_announcement(
            guild,
            _make_bot(),
            "9.9.9",  # no entry in dict for this version
        )
...
def test_migration_adds_release_announcements_enabled_to_guild_configs(temp_db):
    with sqlite3.connect(temp_db) as conn:
        cols = {r[1] for r in conn.execute("PRAGMA table_info(guild_configs)").fetchall()}
