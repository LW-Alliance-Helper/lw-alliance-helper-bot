...
class TestBuildDsMail:
    ...
    def test_placeholders_substituted(self, seeded_db):
        ...
        save_storm_config(TEST_GUILD_ID, "DS", "DS Assignments", template, "America/New_York", 0)
...
class TestBuildCsMail:
    ...
    def test_cs_uses_subs_not_subs_list(self, seeded_db):
        ...
        template = "CS: {zones}\n{subs}\n{time}"
        save_storm_config(TEST_GUILD_ID, "CS", "CS Assignments", template, "America/New_York", 0)
        zones = {"Z1": ["Alice"]}
    ...
    def test_cs_and_ds_templates_independent(self, seeded_db):
        ...
        save_storm_config(TEST_GUILD_ID, "CS", "CS Tab", "CS: {zones}", "America/New_York", 0)
...
class TestCsZoneLabelRendering:
    ...
    def test_subs_key_not_duplicated_as_zone(self, seeded_db):
        ...
        assert "Bob & Carol" not in zones_part, f"Subs key was duplicated as a zone:\n{result}"
    ...
    def test_open_zones_are_skipped(self, seeded_db):
        ...
        save_storm_config(TEST_GUILD_ID, "CS", "CS Assignments", "{zones}", "America/New_York", 0)
        zones = {
            "s1_dc1": ["Alice"],
            "s1_dc2": "(open)",
            "s1_sw1": "",
...
class TestCsZoneStructureConsistency:
    ...
    def test_structure_has_no_duplicate_keys(self):
        ...
        assert len(keys) == len(set(keys)), f"Duplicate keys in CS_ZONE_STRUCTURE: {keys}"
...
class TestFlatSubs:
    ...
    def test_split_legacy_subs_handles_mixed_separators(self):
        ...
        assert _split_legacy_subs("Alice & Bob, Carol - Dave") == ["Alice", "Bob", "Carol", "Dave"]
...
class TestParticipationConfig:
    ...
    def test_participation_config_is_independent_per_event_type(self, seeded_db):
        ...
        save_storm_config(TEST_GUILD_ID, "DS", "DS Assignments", "B", "America/New_York", 0)
        save_storm_config(TEST_GUILD_ID, "CS", "CS Assignments", "B", "America/New_York", 0)
