...
#
# The dedup signature, the quiet window, and the re-nudge that used to live
# here moved to config_health, which does the same job for every feature whose
# config can rot rather than just this one. What stays is the scope labels,
# because naming the three sheets in the alliance's own words is
# transfer-specific copy: config_health only knows them as subject keys.

# The three sheets a poll touches. User-facing: they name which sheet to go
# fix, and they are what transfer_cog registers as config_health subjects.
SHEET_SCOPE_LABELS = {
